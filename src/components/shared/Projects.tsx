    ...
    <div className="w-full">
      ...
      {isStalker ? (
        <div className="mb-12 flex items-center justify-between">
          <h2 className="text-2xl font-mono text-green-500">
            {">"} ls -la ~/projects/deployed
          </h2>
        </div>
      ) : (
      ...
      {/* GRID SECTION */}
      <div className={`grid grid-cols-1 gap-6 ${isStalker ? "md:grid-cols-2" : "lg:grid-cols-2 gap-y-12"}`}>
            ...
            >
              ...
              {isStalker && (
                <div className="flex items-center gap-2 border-b border-green-500/20 bg-green-500/5 px-4 py-2">
                  ...
                  <span className="ml-2 font-mono text-[10px] text-green-500/50">bash: ./run_{project.id}.sh</span>
                </div>
              )}

              {/* MAIN CONTENT BLOCK */}
              <div className={`${isStalker ? "p-6" : "p-8 sm:p-10 flex flex-col h-full"}`}>
                ...
                <div className="flex items-start justify-between mb-8">
                  {isStalker ? (
                    ...
                    // Founder Meta (Asymmetrical & Clean)
                    <div>
                  ...
                  {/* Links Block - Upgraded to SocialIcon */}
                  <div className={`flex items-center gap-3 ${isStalker ? "" : "opacity-50 group-hover:opacity-100 transition-opacity"}`}>
                    {project.links.github && (
                      <SocialIcon 
                        ...
                        style={{ height: isStalker ? 34 : 30, width: isStalker ? 34 : 30 }}
                    ...
                    {project.links.live && (
                      <Link href={project.links.live} target="_blank" className={`flex items-center justify-center transition-all hover:scale-110 ${isStalker ? "text-green-400" : "text-zinc-400 hover:text-white"}`}>
                        {isStalker ? <ExternalLink size={26} /> : <ArrowUpRight size={28} strokeWidth={1.5} />}
                      </Link>
                    )}
                  </div>
                </div>
                ...
                {/* Descriptions */}
                <div className={`flex-1 mb-8 space-y-4 ${isStalker ? "font-mono text-sm" : "font-sans text-base"}`}>
                  {activeData.description.map((desc, i) => (
                    <div key={i} className={`flex items-start ${isStalker ? "gap-3" : "gap-0"}`}>
                      {isStalker && (
                ...
                {/* Tech Stack */}
                <div className={`flex flex-wrap gap-2 pt-6 border-t ${isStalker ? "border-white/5" : "border-white/[0.06]"}`}>
                    ...
                    <span 
                      ...
                      className={`px-3 py-1.5 transition-colors ${
                        isStalker 
                          ...
                          : "text-[10px] rounded-sm bg-transparent border border-white/10 text-zinc-400 font-mono tracking-widest uppercase group-hover:border-white/20 group-hover:text-zinc-300"
                      }`}
                    >
                      {tag}
                    </span>
